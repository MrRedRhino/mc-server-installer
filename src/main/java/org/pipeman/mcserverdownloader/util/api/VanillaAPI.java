...
public class VanillaAPI {
    public static ArrayList<VanillaVersion> getVersions() throws IOException {
        ...
        ArrayList<VanillaVersion> filteredVersions = new ArrayList<>();

        for (int i = 0; i < allVersions.length(); i++) {
            JSONObject version = allVersions.getJSONObject(i);
            if (version.getString("type").equals("release")) {
                filteredVersions.add(new VanillaVersion(version.getString("url"), version.getString("id")));
    ...
    public static URL getDownloadURL(VanillaVersion v) throws IOException {
